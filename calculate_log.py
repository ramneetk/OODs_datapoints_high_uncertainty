## Code from https://github.com/pokaxpoka/deep_Mahalanobis_detector
...
def get_curve(dir_name, stypes = ['Baseline', 'Gaussian_LDA']):
    ...
    for stype in stypes:
        ...
        num_tn = num_n-fp[stype][tpr95_pos]
        if num_tn==0:
            threshold = novel[0]-1
        else:
            threshold = novel[num_tn-1]

    return tp, fp, tnr_at_tpr95, threshold

def metric(dir_name, stypes = ['Bas', 'Gau'], verbose=False):
    tp, fp, tnr_at_tpr95, ood_threshold = get_curve(dir_name, stypes)
    results = dict()
